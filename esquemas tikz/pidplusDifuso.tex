...
\begin{tikzpicture}[auto, >=latex']
    ...
    \node [block1, right = of errorjoint] (filtro) {$\frac{1}{0.1s + 1}$};
    \coordinate[node distance=1cm, above = of filtro] (empty2) {};
    
    \node [fuzzyBlock, minimum height=2em, minimum width=3em, node distance=0.6cm, draw, right = of empty2] (controller) {};
    \node [block1, right = of filtro] (controllerpid) {$PID$};
    ...
    \node [block1, node distance=1.2cm, below = of sum2] (sensor){$H(s)$};
    

    \draw [draw,->] (input) node[above right] {$sp$} -- (sum);
    \draw [-] (sum) -- (errorjoint);
    \draw [-] (errorjoint) -- (filtro);
    \draw [-] (errorjoint) |- (empty2);
    \draw [->] (filtro) -- (controllerpid);
    \draw [->] (empty2) -- (controller);
    ...
    \draw[red,thick,dashed] ($(filtro.north west)+(-0.2,0.1)$)  rectangle ($(filtro.south east)+(0.2,-0.1)$);
