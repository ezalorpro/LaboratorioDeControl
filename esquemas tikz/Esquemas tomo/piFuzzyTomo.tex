...
\begin{tikzpicture}[auto, >=latex']
    ...
    \node [block1, minimum height=2em, minimum width=2em, node distance=0.6cm, below = of empty1] (derivada) {$\frac{d}{dt}$};
    \coordinate[node distance=0.2cm, right = of derivada] (empty3) {};

    \node [fuzzyBlock, minimum height=2em, minimum width=3em, node distance=1cm, draw, right = of empty1] (controlador) {};
    ...
    \coordinate[node distance=1.2cm, below = of proceso] (empty2) {};    

    \draw [->] (input) node[above right] {$sp$} -- (sum);
    \draw [-] (sum) -- (errorbranch);
    \draw [-] (errorbranch) -- (empty1);
    \draw [->] (errorbranch) |- (derivada);
    \draw [-] (derivada) -- (empty3);
    \draw [->] (empty3) |- ([yshift=-1em]controlador) node[pos=0.4, below right] {$\Delta e(t)$};
    \draw [->] (empty1) -- (controlador) node[pos=0.5, above left] {$e(t)$};
